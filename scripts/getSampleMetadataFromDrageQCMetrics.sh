...
echo "mysql --defaults-file=/home/am5153/.my.cnf -u am5153 -h prodseq -e \"USE sequenceDB; select pt.sample_internal_name, pt.exomeKit, dqc.seqGender, dqc.OverallCoverage from prepT pt, dragen_qc_metrics dqc where dqc.pseudo_prepid = pt.experiment_id AND pt.status = 'In DragenDB' AND pt.sample_internal_name in (${sample});\"" > cmds.sql.${file}.sh
